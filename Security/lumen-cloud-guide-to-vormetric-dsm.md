...
* Vormetric DSM Deployed into your account in a [dedicated network vlan](../Network/Lumen Cloud/creating-and-deleting-vlans.md). Customers leveraging an account hierarchy are advised to place the DSM in the parent account as multi-tenant capabilities of the DSM allow for logical separation of policies, key, security administrators.
...
In multisite topologies, DSM high availability spans physical locations and leverage our [Cross Data Center Firewall Services](../Network/Lumen Cloud/creating-cross-data-center-firewall-policies.md). This allows DR Host Agents to maintain access to their encryption services even in the event of a total data center interruption.
...
6. Connect securely to our **failover DSM** via [IPSEC](../Network/Lumen Cloud/creating-a-self-service-ipsec-site-to-site-vpn-tunnel.md) or [Client VPN](../Network/Lumen Cloud/how-to-configure-client-vpn.md) and use SSH to login as **cliadmin**.
...
Set up the Automatic Backup feature to protect the configuration settings as well the encryption keys and policies. To do this, you must access a File Server (a UNIX or Windows host) that is [network accessible](../Network/Lumen Cloud/connecting-data-center-networks-through-firewall-policies.md) by the DSM to store the backup files. Automatic DSM database backup is configured in the Automatic Backup window.
