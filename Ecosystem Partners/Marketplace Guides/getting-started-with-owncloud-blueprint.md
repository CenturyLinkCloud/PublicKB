...
![ownCloud logo](https://bitnami.com/assets/stacks/owncloud/img/owncloud-stack-220x234.png)
...
1. **Locate the ownCloud Stack Blueprint**
  1. Starting from the CenturyLink Control Panel, navigate to the Blueprints Library.
  2. Search for “ownCloud” in the keyword search on the right side of the page.
  3. Locate the 'Install ownCloud on Linux' Blueprint
...
### Deploy ownCloud to an existing server (alternate option)
ownCloud Stack is available as a Script Package for deployment on an existing server based on your own sizing requirements or to support more advanced configurations such as customized Blueprint Workflows to repeatably deploy multiple stacks on servers.

#### Steps to deploy to an existing server
1. **Deploy or Identify an Existing Server**
Identify the server targeted for ownCloud installation.  The Operating system must be supported by the Script Package.  See the [Creating a new enterprise cloud server](../../Servers/creating-a-new-enterprise-cloud-server.md) KB for more information on completing this step.

2. **Select to Execute the Package on a Server Group**
  1. Packages can be executed on one more more servers in a Group.  Search for the public script package named **Install ownCloud on Linux**.
